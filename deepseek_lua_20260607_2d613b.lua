--[[
    PREMIUM MULTIHACK - VERSIÓN MÓVIL
    Sin Device Spoofer, solo funciones esenciales
    GUI táctil 100% funcional
]]
...
CreateToggle(ScrollContainer, "Silent Aim", function() return Settings.SilentAim end, function(v) Settings.SilentAim = v end, "Apuntado automático (toca pantalla para activar)", order); order = order + 1
CreateToggle(ScrollContainer, "Toggle Mode", function() return Settings.AimToggleMode end, function(v) Settings.AimToggleMode = v end, "Toggle ON/OFF | Desactivado = mantener toque", order); order = order + 1
CreateToggle(ScrollContainer, "Team Check", function() return Settings.AimTeamCheck end, function(v) Settings.AimTeamCheck = v end, "No apunta a compañeros", order); order = order + 1
CreateToggle(ScrollContainer, "Wall Check", function() return Settings.AimWallCheck end, function(v) Settings.AimWallCheck = v end, "Solo apunta si es visible", order); order = order + 1
CreateToggle(ScrollContainer, "Wallbang", function() return Settings.Wallbang end, function(v) Settings.Wallbang = v end, "Atraviesa muros (anula Wall Check)", order); order = order + 1
CreateDropdown(ScrollContainer, "Hit Part", {"HitboxHead", "Head", "UpperTorso", "HumanoidRootPart", "LowerTorso"}, function() return Settings.HitPart end, function(v) Settings.HitPart = v end, "Parte del cuerpo a apuntar", order); order = order + 1

-- Sección ESP
local espSection = CreateSection("👁️ VISUALS / ESP", order); order = order + 1
CreateToggle(ScrollContainer, "Enable ESP", function() return Settings.ESP end, function(v) Settings.ESP = v end, "Marca a los jugadores con Highlight", order); order = order + 1
CreateToggle(ScrollContainer, "ESP Team Check", function() return Settings.ESPTeamCheck end, function(v) Settings.ESPTeamCheck = v end, "No marca a compañeros de equipo", order); order = order + 1
CreateToggle(ScrollContainer, "ESP Wall Check", function() return Settings.ESPWallCheck end, function(v) Settings.ESPWallCheck = v end, "ESP se oculta detrás de paredes", order); order = order + 1

-- Sección Movement
local moveSection = CreateSection("🌀 MOVEMENT", order); order = order + 1
CreateToggle(ScrollContainer, "Fly", function() return Settings.Fly end, function(v) Settings.Fly = v end, "Volar con WASD + Espacio/Control", order); order = order + 1
CreateSlider(ScrollContainer, "Fly Speed", 10, 200, function() return Settings.FlySpeed end, function(v) Settings.FlySpeed = v end, "Velocidad del vuelo", order); order = order + 1
CreateToggle(ScrollContainer, "Noclip", function() return Settings.Noclip end, function(v) Settings.Noclip = v end, "Atraviesa paredes y objetos", order); order = order + 1
...
local function UpdateCanvas()
    ...
    for _, child in pairs(ScrollContainer:GetChildren()) do
        if child:IsA("Frame") and child ~= UILayout and child ~= aimSection and child ~= espSection and child ~= moveSection then
            totalHeight = totalHeight + child.AbsoluteSize.Y + 8
        end
    end
    -- Añadir altura de las secciones
    totalHeight = totalHeight + 40 + 40 + 40 + 20 -- Secciones Aimbot, ESP, Movement + espacio
...
-- Cerrar con tecla F4 (PC) o botón back (móvil)
